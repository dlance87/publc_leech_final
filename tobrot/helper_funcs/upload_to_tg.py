...
async def upload_single_file(message, local_file_name, caption_str, from_user):
    ...
    try:
        ...
        if local_file_name.upper().endswith(( "WEBM")):
            ...
            # send video
            directory_file_path = os.path.join(DOWNLOAD_LOCATION)
            # create download directory, if not exist
            if not os.path.isdir(directory_file_path):
                os.makedirs(directory_file_path)
                file_path = os.path.join(directory_file_path, local_file_name)
                
            for filename in os.listdir(file_path):
                old_file_name = filename
                new_file_name  = '@movieserieshub.' +  filename
